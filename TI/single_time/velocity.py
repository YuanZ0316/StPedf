...
def get_neigh_trans(
    ...
    if n_neigh_pos:
        ...
        neigh_pos = neigh_pos[:, 1:]  # 排除自身

    # SEDR嵌入邻居
    if n_neigh_embed:
        if "SEDR" not in adata.obsm:
            raise ValueError("SEDR embedding not found in adata.obsm")
        
        nn_embed = NearestNeighbors(n_neighbors=n_neigh_embed, n_jobs=-1)
        nn_embed.fit(adata.obsm["SEDR"])
        _, neigh_embed = nn_embed.kneighbors(adata.obsm["SEDR"])
        neigh_embed = neigh_embed[:, 1:]  # 排除自身
