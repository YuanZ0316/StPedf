...
class InnerProductDecoder(nn.Module):
    ...
    def forward(self, z, mask=None):
        adj_full = torch.mm(z, z.t()) 
        adj_full = self.act(adj_full)  
        if mask is not None and mask.is_sparse:
            ...
            pred_values = adj_full[rows, cols] 
            return pred_values 

        return adj_full 


class StPedf_module(nn.Module):
    def __init__(
            self,
            input_dim,
            feat_hidden1=64,
        ...
        super(StPedf_module, self).__init__()
    ...
    def forward(self, x, adj):
        ...
        self.latent_z = z  
        self.reconstructed_adj = self.dc(z, adj)  
        ...
        if self.training:
            ...
            self.reconstructed_adj = self.dc(z) 
...
class StPedf_impute_module(nn.Module):
    def __init__(
            self,
            input_dim,
            feat_hidden1=64,
        ...
        super(StPedf_impute_module, self).__init__()
        self.input_dim = input_dim
        self.feat_hidden1 = feat_hidden1
        self.feat_hidden2 = feat_hidden2
        self.gcn_hidden1 = gcn_hidden1
    ...
    def forward(self, x, adj):
        ...
        reconstructed_adj = self.dc(z, adj)  
