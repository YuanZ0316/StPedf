...
from .StPedf_module import StPedf_module, StPedf_impute_module
...
class StPedf:
    def __init__(
        ...
        if self.mode == 'clustering':
            self.model = StPedf_module(self.input_dim).to(self.device)
        elif self.mode == 'imputation':
            self.model = StPedf_impute_module(self.input_dim).to(self.device)
        else:
    ...
    def train_with_dec(
        ...
        # latent_z, q, feat_x, gnn_z, reconstructed_adj
