...
class InnerProductDecoder(nn.Module):
    def __init__(self, dropout, act=torch.sigmoid):  # 修改激活函数为 torch.sigmoid
        super(InnerProductDecoder, self).__init__()
        self.dropout = dropout
        self.act = act

    def forward(self, z, mask=None):
        """生成完整的邻接矩阵（训练时使用掩码筛选边，推理时返回完整矩阵）"""
        # 计算所有节点对的内积
        adj_full = torch.mm(z, z.t())  # 形状 [n_nodes, n_nodes]
        adj_full = self.act(adj_full)  # 应用激活函数
        # 训练时返回掩码对应的边列表
        if mask is not None and mask.is_sparse:
            mask_coalesced = mask.coalesce()
            rows = mask_coalesced.indices()[0]
            cols = mask_coalesced.indices()[1]
            pred_values = adj_full[rows, cols]  # 提取掩码对应的边
            return pred_values  # 一维张量（用于训练损失计算）

        return adj_full  # 返回完整矩阵（用于推理）
...
class SEDR_module(nn.Module):
    def __init__(
        ...
        self.dc = InnerProductDecoder(self.p_drop, act=torch.sigmoid)  # 修改激活函数为 torch.sigmoid
        # DEC cluster layer
    ...
    def forward(self, x, adj):
        ...
        # 存储关键属性
        self.latent_z = z  # 存储潜在表示
        self.reconstructed_adj = self.dc(z, adj)  # 存储重构邻接矩阵
        ...
        # 生成邻接矩阵（训练时传入mask，推理时不传）
        if self.training:
            self.reconstructed_adj = self.dc(z, adj)  # 训练时返回边列表
        else:
            self.reconstructed_adj = self.dc(z)  # 推理时返回完整矩阵
...
class SEDR_impute_module(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        gnn_z = self.reparameterize(mu, logvar)
        z = torch.cat((feat_x, gnn_z), 1)
        de_feat = self.decoder(z)  # 特征重构

        # 生成重构邻接矩阵
        reconstructed_adj = self.dc(z, adj)  # 使用 InnerProductDecoder
        ...
        # 返回重构邻接矩阵
        return z, mu, logvar, de_feat, q, feat_x, gnn_z, loss, reconstructed_adj
